...
import { FileUtil, FileNotFoundError, FileIsAlreadyExistedError, IsDirectoryError, SimpleCopy, CopyStrategy, CopyWithEncrypt, CopyWithDecrypt } from "../../util/FileUtil";
...
describe("Test FileUtil", () => {
  ...
  describe("Test rename", () => {
    ...
    test("Test rename the directory, but the new path is already existed", async () => {
      try {
        ...
        expect(error).toBeInstanceOf(FileIsAlreadyExistedError);
      } finally {
  ...
  describe("Test copy", () => {
    ...
    describe("Test success to copy file with strategy", () => {
      test(`Test success to copy file with strategy`, async () => {
        ...
        let strategies: CopyStrategy[] = [new SimpleCopy(fileFrom, to), new CopyWithEncrypt(publicKeyPath, fileFrom, to), new CopyWithDecrypt(privateKeyPath, fileFrom, to)];
        let inputs: string[] = await Promise.all([content, content, EncryptUtil.encryptString(publicKeyPath, content).then(result => result + "\n")]);
        let outputs: string[] = await Promise.all([content, content, content]);
    ...
    describe("Test success to copy directory", () => {
      ...
      for (let c of cases) {
        const to = c.to;
        let strategies: CopyStrategy[] = [new SimpleCopy(dirFrom, to), new CopyWithEncrypt(publicKeyPath, dirFrom, to), new CopyWithDecrypt(privateKeyPath, dirFrom, to)];
        let inputs: any = Promise.all([content, content, EncryptUtil.encryptString(publicKeyPath, content).then(result => result + "\n")]);
        for (let i = 0; i < strategies.length; i++) {
  ...
  describe("Test copyFileWithEncryption", () => {
    ...
    test("Test copyFileWithEncryption with a directory", async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(IsDirectoryError);
      }
    });
  });
  ...
  describe("Test copyFileWithDecryption", () => {
    ...
    test("Test copyFileWithDecryption with a directory", async () => {
      ...
      try {
        await FileUtil.copyFileWithDecryption(publicKeyPath, fileFrom, to);
      } catch (error) {
        expect(error).toBeInstanceOf(IsDirectoryError);
      }
    });
